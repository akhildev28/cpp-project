...
class Student
{
    ...
    // Change Token to 1 when student took a book
    ...
    // Change Token to 0 when student return a book

     void resettoken(int token)
    {
        token--;
    }
       ...
       fstream getid;
        ...
        while (!getid.eof())
        {
            ...
            if (getid.read((char*)&bk,sizeof(Book)))
            {
                ...
                int BookNoint = stoi(bk.BookNo);
                BookNoint ++;
                std::sprintf(bk.BookNo, "%d", BookNoint);
    ...
    void write_student()
{
    ...
    {  
        ...
        //creating file object
         fstream getid;
        //open the file student.dat to getid
        getid.open("student.dat",ios::binary|ios::in);
        


        //CODE FOR INGRIMENT ID *********
        
        
        
        //while loop for get the end of file
       /* while (!getid.eof())
        {
            ...
            if (getid.read((char*)&st,sizeof(Student)))
            {
                ...
                int  StNoint = stoi(st.LibraryId);//code for converting to integer and assing to SttNoint

                StNoint ++;//ingriment 1 
                std::sprintf(st.LibraryId, "%d", StNoint);//code for converting integer to char
